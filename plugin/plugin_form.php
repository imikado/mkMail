...
class plugin_form{
	...
	*/
	...
	/** 
	* retourne une liste de champs checkbox
	* @access public
	* @param string $sName nom du champ
	* @param array @tValue tableau des valeurs de champ checkbox
	* @param array $tOption options des champs 
	*/
	public function getListCheckbox($sName,$tValue,$tOption=null){
		$tCurrentValue=$this->getValue($sName);
		
		$sHtml=null;
	
		foreach($tValue as $sValue => $sLabel){
			$sHtml.='<input type="checkbox" name="'.$sName.'[]" '; 
			if(in_array($sValue,$tCurrentValue)){ 
				$sHtml.=' checked="checked"'; 
			} 
			$sHtml.=' value="'.$sValue.'" '.$this->getOption($tOption).'/>'.$sLabel.' ';
		}
