<?php
class model_tags extends abstract_model{
	
	protected $sClassRow='row_tags';
	...
	public function findByName($sName){
		return $this->findOneSimple('SELECT * FROM '.$this->sTable.' WHERE name=?',$sName );
	}
	
	public function getSelect(){
		$tRow=$this->findManySimple('SELECT * FROM '.$this->sTable);
		$tIndex=array();
		if($tRow){
			foreach($tRow as $oRow){
				$tIndex[$oRow->id]=$oRow->name;
			}
		}
		return $tIndex;
	}
...
class row_tags extends abstract_row{
	
	protected $sClassModel='model_tags';
