<?php
class model_contacts extends abstract_model{
	
	protected $sClassRow='row_contacts';
	
	protected $sTable='contacts';
	...
	public function getSelect(){
		...
		foreach($tab as $oRow){
			...
			$sContact=$oRow->firstname.' '.$oRow->lastname;
			if($oRow->firstname==''){
				$sContact=$oRow->email;
				
			}
			
			$tSelect[ $oRow->id ]=$sContact;
		}
		}
		return $tSelect;
	}
	...
	public function getSelectWithEmpty(){
		$tab=$this->findAll();
		$tSelect=array('pas de filtre');
		if($tab){
		foreach($tab as $oRow){
...
class row_contacts extends abstract_row{
	
	protected $sClassModel='model_contacts';
